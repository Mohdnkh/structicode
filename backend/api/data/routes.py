...
def project_response(item:Project)->ProjectResponse:
    return ProjectResponse(id=item.id,organization_id=item.organization_id,name=item.name,description=item.description,state=item.state,version=item.version,created_by_user_id=item.created_by_user_id,updated_by_user_id=item.updated_by_user_id,created_at=utc(item.created_at),updated_at=utc(item.updated_at))
...
def organizations(user:User=Depends(current_user),db:Session=Depends(get_db)):
    ...
    return [OrganizationResponse(id=org.id,name=org.name,role=membership.role,created_at=utc(org.created_at)) for org,membership in rows]
